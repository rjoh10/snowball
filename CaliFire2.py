...
conditions = [
    df['GIS Calculated Acres'] < threshold / 2,
    (df['GIS Calculated Acres'] >= threshold / 2) & (df['GIS Calculated Acres'] <= threshold * 1.5),
    df['GIS Calculated Acres'] > threshold
]
categories = [0,1,2]

df['Fire_Size_Category'] = np.select(conditions, categories, default=-1)
